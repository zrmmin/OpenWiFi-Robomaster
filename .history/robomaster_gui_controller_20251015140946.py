...
class RoboMasterController(tk.Tk):
    ...
    def gamepad_poll_loop(self):
        """ 循环检测手柄输入并发送指令 """
        pygame.init()
        # 关键：即使没有窗口，也需要初始化display模块才能正确处理事件队列
        pygame.display.init()
        ...
        while self.is_gamepad_control_on:
            ...
            if not joystick:
                ...
                if pygame.joystick.get_count() > 0:
                    ...
                    # 线程安全地更新UI
                    self.after(0, lambda: self.log(f"已连接手柄: {name} ({num_axes}个轴)"))
                    self.after(0, lambda: self.gamepad_status_label.config(text=f"手柄: {name}"))
                    self.after(0, lambda: self.update_axis_comboboxes(num_axes))
                else:
                    self.after(0, lambda: self.log("未检测到手柄，请连接..."))
                    self.after(0, lambda: self.gamepad_status_label.config(text="手柄: 未连接"))
                    time.sleep(2)
                    continue
            
            # 2. 事件处理 (手柄插拔等) - 这是pygame线程崩溃的关键点
            try:
                ...
                # 捕获并记录 pygame 事件错误，防止线程崩溃
                self.after(0, lambda: self.log(f"Pygame 事件错误: {e}"))
                time.sleep(1)
                continue
        ...
        # 循环结束，停止机器人并按初始化的逆序退出pygame
        if self.is_connected:
            self.send_command("chassis speed x 0 y 0 z 0;")
        pygame.joystick.quit()
        pygame.display.quit()
        pygame.quit()
        self.after(0, lambda: self.log("手柄控制线程已安全退出。"))
