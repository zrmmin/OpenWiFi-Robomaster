...
try:
    ...
    from robomaster import conn # 导入连接模块
except ImportError:
...
class RoboMasterController(tk.Tk):
    ...
    def connect_robot(self):
        ...
        try:
            # --- 最终方案：手动创建并传入连接对象 ---
            # 新的错误日志显示，Robot()构造函数需要一个连接对象，而不是IP字符串。
            # 我们手动创建一个TCP连接对象，然后将其传递给Robot实例。
            robot_conn = conn.TCPConnection(robot_ip)
            self.ep_robot = robot.Robot(robot_conn)
