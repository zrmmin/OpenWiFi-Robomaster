...
try:
    ...
    # from robomaster import conn # 移除不再需要的导入
except ImportError:
...
class RoboMasterController(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Label(conn_frame, text="本机IP (PC):").grid(row=2, column=0, padx=5, pady=5, sticky="w")
        self.local_ip_entry = ttk.Entry(conn_frame, width=20)
        self.local_ip_entry.grid(row=2, column=1, padx=5, pady=5, sticky="ew")
        self.local_ip_entry.insert(0, "192.168.11.100") # 您PC的IP

        self.connect_btn = ttk.Button(conn_frame, text="连接", command=self.toggle_connection)
        self.connect_btn.grid(row=2, column=2, rowspan=1, padx=5, pady=5, sticky="ns")
    ...
    def connect_robot(self):
        ...
        if not robot_ip or not local_ip:
            self.log("错误：请输入机器人的IP地址和本机的IP地址！")
        ...
        try:
            # --- 最终方案：根据官方文档，同时指定本地IP和目标IP ---
            # 1. 强制SDK使用本机的正确IP地址发起连接
            robomaster.config.LOCAL_IP_STR = local_ip
            # 2. 强制SDK连接到指定的目标机器人IP
            robomaster.config.ep_ip = robot_ip
            # 3. (可能需要)为旧版SDK打上协议补丁
            robomaster.config.DEFAULT_CONN_PROTO = "tcp"
