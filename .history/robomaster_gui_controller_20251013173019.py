...
# --- 视频流与图像处理依赖 ---
# 请确保已安装所需库: pip install opencv-python Pillow h264decoder
try:
    import cv2
    ...
    print("pip install opencv-python Pillow h264decoder")
    print("="*40)
    exit()
    ...
    def receive_video_data(self):
        ...
        try:
            ...
            while self.is_video_on:
                ...
                framedatas = decoder.decode(raw_data)
                for framedata in framedatas:
                    (frame, w, h, ls) = framedata
                    if frame is not None:
                        # 将解码后的裸数据转换为OpenCV图像格式
                        frame_np = np.frombuffer(frame, dtype=np.uint8)
                        frame_np = frame_np.reshape((h, ls // 3, 3))
                        frame_np = frame_np[:, :w, :]
                        
                        # 在GUI上显示图像
                        self.update_video_label(frame_np)
