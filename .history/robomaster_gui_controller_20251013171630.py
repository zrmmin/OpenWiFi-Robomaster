...
import threading
import time
import socket # 导入socket库，用于底层网络通信
...
class RoboMasterController(tk.Tk):
    """
    RoboMaster 图形化控制主窗口 (明文SDK模式)
    """
    def __init__(self):
        super().__init__()
        self.title("RoboMaster GUI 控制器 - [明文SDK模式]")
        self.geometry("600x450") # 简化界面

        self.robot_ip = ""
        self.robot_port = 40923 # 明文SDK默认端口
        self.sock = None
    ...
    def create_widgets(self):
        ...
        ttk.Button(chassis_frame, text="↑\n前进", command=lambda: self.send_command("chassis move x 0.5;")).grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(chassis_frame, text="↓\n后退", command=lambda: self.send_command("chassis move x -0.5;")).grid(row=2, column=1, padx=5, pady=5)
        ttk.Button(chassis_frame, text="←\n左移", command=lambda: self.send_command("chassis move y -0.5;")).grid(row=1, column=0, padx=5, pady=5)
        ttk.Button(chassis_frame, text="→\n右移", command=lambda: self.send_command("chassis move y 0.5;")).grid(row=1, column=2, padx=5, pady=5)
        ttk.Button(chassis_frame, text="↺\n左旋", command=lambda: self.send_command("chassis move z -30;")).grid(row=0, column=0, padx=5, pady=5)
        ttk.Button(chassis_frame, text="↻\n右旋", command=lambda: self.send_command("chassis move z 30;")).grid(row=0, column=2, padx=5, pady=5)
        ...
        ttk.Button(gimbal_frame, text="↑\n向上", command=lambda: self.send_command("gimbal move p 10;")).grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(gimbal_frame, text="↓\n向下", command=lambda: self.send_command("gimbal move p -10;")).grid(row=1, column=1, padx=5, pady=5)
        ttk.Button(gimbal_frame, text="←\n向左", command=lambda: self.send_command("gimbal move y -15;")).grid(row=1, column=0, padx=5, pady=5)
        ttk.Button(gimbal_frame, text="→\n向右", command=lambda: self.send_command("gimbal move y 15;")).grid(row=1, column=2, padx=5, pady=5)
    ...
    def connect_robot(self):
        ...
        try:
            self.log(f"正在创建UDP套接字，目标: {self.robot_ip}:{self.robot_port}")
            self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            
            self.log("发送 'command;' 进入SDK模式...")
            self.sock.sendto(b'command;', (self.robot_ip, self.robot_port))
            # 可以加入接收确认的逻辑，但为简化，先假设成功
            time.sleep(1)

            self.is_connected = True
            self.log("连接成功！已进入SDK明文控制模式。")
    ...
    def disconnect_robot(self):
        ...
        if self.is_connected:
            ...
            self.is_connected = False # 停止心跳线程
        if self.sock:
            self.sock.close()
            self.sock = None
        self.log("连接已断开。")
    ...
    def send_command(self, cmd_str):
        if self.is_connected and self.sock:
            try:
                self.log(f"发送指令: {cmd_str}")
                self.sock.sendto(cmd_str.encode('utf-8'), (self.robot_ip, self.robot_port))
            except Exception as e:
                self.log(f"指令发送失败: {e}")
                self.disconnect_robot()
        else:
    ...
    def send_heartbeat(self):
        """ 每隔5秒发送一次心跳指令，防止机器人退出SDK模式 """
        while self.is_connected:
            self.send_command("robot mode free;") # 发送一个无害指令作为心跳
            time.sleep(5)
